...
class Talk(object):
    ...
    def sendImage(self, to, path):
        message = self.sendMessage(to=to, text=None, contentType = 1)
        self.uploadObjTalk(path=path, type='image', returnAs='bool', objId=message.id)
        return message
    ...
    def sendVideo(self, to, path):
        message = self.sendMessage(to=to, text=None, contentMetadata={'VIDLEN': '60000','DURATION': '60000'}, contentType = 2)
        self.uploadObjTalk(path=path, type='video', returnAs='bool', objId=message.id)
        return message
    ...
    def sendAudio(self, to, path):
        message = self.sendMessage(to=to, text=None, contentType = 3).id
        self.uploadObjTalk(path=path, type='audio', returnAs='bool', objId=message.id)
        return message
    ...
    def sendFile(self, to, path, file_name=''):
        ...
        message = self.sendMessage(to=to, text=None, contentMetadata={'FILE_NAME': str(file_name),'FILE_SIZE': str(file_size)}, contentType = 14)
        self.uploadObjTalk(path=path, type='file', returnAs='bool', objId=message.id, name=file_name)
        return message
